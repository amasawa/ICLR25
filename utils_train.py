...
def checkpoint_training_state(checkpoint_dir, accelerator, ema_model, step: int, run_id: str):
    ...
    if ema_model is not None:
        #print(step)
        ema_checkpoint_path = os.path.join(checkpoint_dir, str(step)+"_emamodel.pt")
            # oscheckpoint_dir / str(step)+"_emamodel.pt"
        torch.save(ema_model.state_dict(), ema_checkpoint_path)
        ema_checkpoint_path_last = os.path.join(checkpoint_dir, "last.pt")
        torch.save(ema_model.state_dict(), ema_checkpoint_path_last)




# def noac_checkpoint_training_state(checkpoint_dir, model, optimizer, ema_model, step: int, run_id: str):
#     if checkpoint_dir is None:
#         return
#     logger.info(f"Checkpointing training state to {checkpoint_dir} at step {step}")
#
#     # 保存模型和优化器状态
#     model_state_path = checkpoint_dir / f"model_state_{step}.pt"
#     optimizer_state_path = checkpoint_dir / f"optimizer_state_{step}.pt"
#     torch.save(model.state_dict(), model_state_path)
#     torch.save(optimizer.state_dict(), optimizer_state_path)
#
#     # 如果使用了EMA模型，也保存它
#     if ema_model is not None:
#         ema_checkpoint_path = checkpoint_dir / f"ema_model_{step}.pt"
#         torch.save(ema_model.state_dict(), ema_checkpoint_path)
#
#     # 保存其他信息
#     with open(checkpoint_dir / "info.json", "w") as f:
...
def make_bfn(cfg: DictConfig):
    ...
    #print(net)
    #summary(net,[(32,32,3),(32,32,3),(1,32)]) #for infoBFN 32
    #summary(net, [(32, 32, 3), (32, 32, 3)])# for BFN 32
    # summary(net,input_data=[torch.randn(1,64,64,3),torch.randn(1,64,64,3),torch.randn(1,32)]) #for infoBFN 64
    # assert False
    if cfg.encoder['class_name'] is not None:
        encoder = make_from_cfg(networks, cfg.encoder)
        #summary(encoder.cuda(),(64,64,3))
        #assert False
        bfn = model.BFNAE(net=net, encoder=encoder,bayesian_flow=bayesian_flow, loss=loss, KL=cfg.KL)
        #summary(bfn.cuda(),(28,28,1))
        print("--------Training BFNvae--------")
    else:
        bfn = model.BFN(net=net, bayesian_flow=bayesian_flow, loss=loss)
        print("--------Training BFN--------")
        #assert False
        #summary(bfn.cuda(),(28,28,1))
    #assert False, "下面的还没实现"
    #assert False
    return bfn
